#ifndef PSTRINGMATCHING_SIMILARITY_JOIN_JACCARD_JOIN_HPP
#define PSTRINGMATCHING_SIMILARITY_JOIN_JACCARD_JOIN_HPP
...
namespace similarity_join {
  ...
  boost::python::dict jaccard_join(const std::vector< std::set <T> > & tokens_1, const std::vector< std::set <T> > & tokens_2, float threshold) {
    ...
    for (std::size_t length_1 = 1; length_1 < length_index_1.size(); length_1++) {
      ...
      for (std::size_t length_2 = 0; length_2 < length_index_2.size(); length_2++) {
        ...
        #pragma omp parallel
        {
          ...
          for (i = istart; i < iend; i++) {
            ...
            if (sim >= threshold) {
              buckets[ID].emplace_back(idx_1, idx_2, sim);
            }
            idx_2 += 1;
            if (idx_2 == M) {
              idx_1 += 1;
